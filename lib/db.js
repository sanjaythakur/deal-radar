...
async function ensureInit() {
  ...
  _initPromise = (async () => {
    ...
    _initPromise = null;
    // pg surfaces connection failures as an `AggregateError` whose default
    // toString hides the underlying ECONNREFUSED / ENOTFOUND. Unwrap and
    // rethrow with a hint that points devs at the local Postgres setup.
    ...
    if (inner) {
      ...
      const wrapped = new Error(
        `Database init failed${code}: ${inner.message}${hint}`,
      );
      wrapped.cause = err;
      throw wrapped;
    }
